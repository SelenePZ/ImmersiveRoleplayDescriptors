...
ISUICreateScene = ISCollapsableWindowJoypad:derive("ISUICreateScene");

local FONT_HGT_SMALL = getTextManager():getFontHeight(UIFont.Small)

--************************************************************************--
--** ISUICreateScene:initialise
--**
--************************************************************************--

function ISUICreateScene:initialise()
...
--    self.winwo:setY(self.y);
    ...
    if not self.locked then
        self.deleteButton = ISButton:new(self.entry.x, self.entry:getBottom() + 4, btnHgt2, btnHgt2, "", self, ISUICreateScene.onClick);
        ...
        self.lockButton = ISButton:new(self.deleteButton.x + self.deleteButton.width + 3, self.deleteButton.y, btnHgt2, btnHgt2, "", self, ISUICreateScene.onClick);
    ...
    if self.numberOfPages > 1 then
        ...
        self.nextPage = ISButton:new(self.width / 2 + 3, self.pageLabel.y, btnHgt2, btnHgt2, ">", self, ISUICreateScene.onClick);
        ...
        self.previousPage = ISButton:new(self.nextPage.x - btnHgt2 - 3, self.nextPage.y, btnHgt2, btnHgt2, "<", self, ISUICreateScene.onClick);
    ...
    self.yes = ISButton:new((self:getWidth() / 2) - btnWid - 5, bottom + 5, btnWid, btnHgt, getText("UI_Ok"), self, ISUICreateScene.onClick);
    ...
    self.no = ISButton:new((self:getWidth() / 2) + 5, bottom + 5, btnWid, btnHgt, getText("UI_Cancel"), self, ISUICreateScene.onClick);
...
function ISUICreateScene:close()
    self:destroy()
end

function ISUICreateScene:destroy()
...
function ISUICreateScene:setJoypadButtons(joypadData)
...
function ISUICreateScene:onClick(button)
...
function ISUICreateScene:prerender()
    self.pinButton:setVisible(false);
...
--************************************************************************--
--** ISUICreateScene:render
--**
--************************************************************************--
function ISUICreateScene:render()
...
function ISUICreateScene:onGainJoypadFocus(joypadData)
...
function ISUICreateScene:onJoypadDown(button, joypadData)
...
function ISUICreateScene:onJoypadDirUp(joypadData)
...
function ISUICreateScene:onJoypadDirDown(joypadData)
    if self.joypadIndexY == #self.joypadButtonsY then
        self:clearJoypadFocus(joypadData)
...
--**
--************************************************************************--
function ISUICreateScene:new(x, y, width, height, target, onclick, character, notebook, defaultEntryText, title, lineNumber, editable, numberOfPages)
    local o = {}
